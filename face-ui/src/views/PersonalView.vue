<template>
    <div>
        ...
        <div v-if="user">
            <p>用户ID: {{ user.id }}</p>
            <p>用户头像路径: <img :src="user.user_face_path ? user.user_face_path : '无'" style="max-width: 200px;"></p>
        ...
        <el-dialog :visible.sync="dialogVisible">
            ...
            <p>用户组ID:
                <el-select v-model="user.user_group_id" placeholder="请选择">
                    <el-option v-for="group in userGroups" :key="group.id" :label="group.name"
...
export default {
    data() {
        return {
            user: {
                id: null,  // 用户ID
                user_face_path: null,  // 用户头像路径保存在user_face_path属性中
                user_group_id: null,  // 用户组ID
                user_id: null,  // 用户登录ID
                user_isadmin: null,  // 是否管理员
                user_name: null,  // 用户名
